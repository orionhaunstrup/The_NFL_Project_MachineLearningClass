...
def find_cluster_players(chosen_player, years_revealed, datacube_filename):
    ## This function (which is new) takes in the test player
    ## and uses K-Means clustering on the new transformed data cubes
    ## to find the closest cluster of players, with the appropriate
    ## career lengths
    ...
    # Now, with the Jagged Cube issue solved, we can finally run
    # a real and meaningful version of K-Means on the data!

    kmeans = KMeans(n_clusters=20, n_init=10, random_state=333)
                ## 20 was chosen after a number of small experiments
                ## as it showed the most promising results
    kmeans.fit(training_vectors)
    
    # Find the correct cluster
    cluster_id = kmeans.predict(test_vector.reshape(1, -1))[0]

    # Find all players in that cluster=
    training_labels = (kmeans.labels_)

    cluster_players = []

    for player_name, label in zip(training_names, training_labels):
        if label == cluster_id:
            cluster_players.append(player_name)

    return cluster_players
...
def main():
    ...
    for experiment_num in range(1, NUM_EXPERIMENTS + 1):
        ...
        datacube_filename, chosen_player = (
            random.choice(eligible_players))
        cluster_players = (find_cluster_players(
        ...
        full_neighbor_careers = (get_full_careers_of_cluster_players(
            cluster_players, datacube_filename))

        chosen_player_percentiles = (get_chosen_player_percentiles(
            chosen_player, datacube_filename))

        median_future_seasons = (predict_num_future_seasons(
        ...
        ## This adds a small progress bar, so we don't lose our minds
        if experiment_num % 250 == 0:
