...
def find_pca_neighbors(chosen_player, years_revealed,
    ...
    data = PRECOMPUTED[(datacube_filename, years_revealed)]

    player_names = data["player_names"]
    vectors = data["vectors"]

    # Locate and remove the test player
    chosen_index = player_names.index(chosen_player)
    test_vector = vectors[chosen_index]
    training_names = (
        player_names[:chosen_index] + player_names[chosen_index + 1:])
    training_vectors = np.delete(vectors, chosen_index, axis=0)

    # Perform the PCA
    pca = PCA(n_components=PCA_VARIANCE)
    pca.fit(training_vectors)
    training_pca = pca.transform(training_vectors)
    test_pca = pca.transform(test_vector.reshape(1, -1))[0]

    # Compute distances
    player_distance_list = []
    
    for i, player_vector in enumerate(training_pca):
        distance = np.linalg.norm(player_vector - test_pca)
        player_distance_list.append((training_names[i], distance))

    player_distance_list.sort(key=lambda x: x[1])

    return player_distance_list[:num_neighbors]
...
def distance_list_of_lists(list_of_lists1, list_of_lists2):
    ...
    for i in range(len(list_of_lists1)):
        for j in range(len(list_of_lists1[0])):
            ...
            total += (float(list_of_lists1[i][j]) - float(
                list_of_lists2[i][j])) ** 2
...
def get_full_careers_of_cluster_players(cluster_players, datacube_filename):
    ...
    for player in cluster_players:
        ...
        career_percentiles = [float(row[-1])for row in rows]
...
def main():
    ...
    print(f"Running {NUM_EXPERIMENTS:,} PCA experiments...")

    for experiment_num in range(1, NUM_EXPERIMENTS + 1):
        years_revealed = random.randint(3,10)
        eligible_players = [(datacube_filename, player)
            for (datacube_filename, player, career_length) in all_players
            if career_length >= years_revealed]
        datacube_filename, chosen_player = (random.choice(eligible_players))
        closest_players = (find_pca_neighbors(chosen_player, years_revealed,
                                              datacube_filename))
        ...
        full_neighbor_careers = (get_full_careers_of_cluster_players(
                [
                    player
                    for player, distance
                    in closest_players
        ...
        median_future_seasons = (
            predict_num_future_seasons(full_neighbor_careers, years_revealed))

        predicted_percentiles = (
            predict_future_percentiles(full_neighbor_careers, years_revealed,
                                       median_future_seasons))
        ...
        retirement_error, prediction_error = (
            score_prediction(predicted_percentiles, actual_future_percentiles))
        ...
        ## Adding a progress bar, so we don't lose our minds waiting
        if experiment_num % 10 == 0:

            percent = (100.0 * experiment_num / NUM_EXPERIMENTS)
    ...
    print("Final Results:")
    print()
    print("Mean Retirement Error:", statistics.mean(retirement_errors))
    print()
    print("Mean Prediction Error:", statistics.mean(prediction_errors))
    print()




### This is a little less than professional, to simply rerun main()
### over and over with a global varibale changed, but considering that
### this code will only run once ever, and is being performed carefully,
### I'd say let's sprinkle some parmesan on the spaghetti, and let's eat! 
...
print()
print()
print("PCA_VARIANCE = " + str(PCA_VARIANCE))
print()
print()
main()
